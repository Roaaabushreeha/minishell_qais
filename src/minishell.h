...
/*   Updated: 2025/05/21 19:17:05 by jalqam           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int								execute_commands_pip(t_cmd *cmd, t_env *env,
									t_token *tokens, int prev_pipe[2]);
int								handle_fork_error(t_cmd *cmd, t_env *env,
									t_token *tokens, t_pipe_fds *pipes);
char							*process_command_arg(t_token *token,
									int is_first_arg, t_env *env);
...
int								init_handle_output_redirection(t_cmd *cmd,
									const char *filename, int is_append);
int								save_stdout_and_redirect(t_cmd *cmds,
									int *saved_stdout);
int								init_handle_input_redirection(t_cmd *cmd,
									const char *filename);
int								handle_single_command(t_cmd *cmd, t_env *env,
...
void							clean_single(t_cmd *cmd, int saved_stdout,
									int status);
...
void							write_heredoc_expanded(int fd, char *buffer,
									char *expanded, t_env *env);
int								check_heredoc_exit(char *buffer, t_env *env);
void							write_heredoc_line(int fd, char *buffer,
									int quoted, t_env *env);
char							*get_key(char *var);
int								is_valid_export(char *arg);
void							handle_invalid_export(char *arg, t_env *env,
									int *i);
int								handle_existing_var(t_env *env, char *key,
									char *arg, int *i);
int								add_new_var(t_env **env, t_env **last,
									char *arg, int *i);
void							handle_quote_states(char c, int *in_double,
									int *in_single);
int								check_token_quotes(t_token *temp);
char							*handle_dollar_param(char *str, int *i);
int								finalize_heredoc(t_cmd *cmd, t_env *env,
									int result);
char							*handle_mixed_quotes(t_token *token);
char							*ft_strjoin_char(char *str, char c);
char							*get_exit_status(t_env *env);
char							*append_char_to_result(char *result, char c);
char							*handle_dollar_expander(t_token *token,
									t_env *env);
char							*handle_twoquotes(t_token *token);
void							cleanup_resources(t_env *env, t_token *tokens,
									t_cmd *temp);
void							split_quotes(char c, int *in_quotes,
									char *quote_char);
...
void							split_chars(char *word, char *new_word,
									size_t *i, size_t *j);
t_cmd							*command(int cmd_num, t_token **token,
									t_env *env);
...
void							exit_command(t_cmd *cmds, t_env *env,
									t_token *token);
...
char							*handle_mixed_quotes(t_token *token);
void							cleanup_iteration(char *readline_shell,
									char *pwd, t_token *tokens, t_cmd *cmd);
...
void							append_node(t_env **env, t_env **last,
									t_env *new_node);
...
int								handle_heredoc(char *delimiter, t_cmd *cmd,
									t_env *env);
void							expand_echo_args(t_cmd *cmds, t_env *env);
int								initredirction(t_token *token, t_cmd *cmd,
									t_env *env);
int								builtin_commands(t_cmd *cmds, t_env *env,
									int *flag);
...
int								get_cmd_execution(t_cmd *cmd, t_env *env,
									t_token *token);
int								handle_pipe_case(t_cmd *cmd, t_env *env,
									t_token *token);
int								handle_builtin_case(t_cmd *cmd, t_env *env,
									t_token *token);
int								handle_pipe_execution(t_cmd *cmd, t_env *env,
									t_token *token);
int								handle_regular_execution(t_cmd *cmd, t_env *env,
									t_token *token);
int								handle_builtin_execution(t_cmd *cmd, t_env *env,
									t_token *token);
int								execute_command_temp(t_cmd *cmd, t_env *env,
									t_token *token);
int								handle_null_readline(t_env *env,
									char *readline_shell);
int								execute_and_cleanup(char *readline_shell,
									t_env *env, t_token *tokens);
int								process_input(char *readline_shell, t_env *env);
int								init_minishell(int argc, char *argv[],
									char *envp[], t_env **env);
int								handle_env_var_expansion(t_expander *s,
									t_env *env);
...
int								update_quote_state(char c, int *in_dq,
									int *in_sq);
char							*append_char_to_result(char *result, char c);
char							*handle_mixed_quotes(t_token *token);
void							copy_inside_quotes(const char *str,
									char *result, int *i, int *j);
void							copy_outside_quotes(const char *str,
									char *result, int *i, int *j);
